...
do
  ...
  do
    ...
    ########
    $compiler -Wno-everything -S -DINCLUDEMAIN -emit-llvm -fno-discard-value-names -g -I"$root" "$c_f" -o "$bc_f"
    #llvm-as "$bc_f" -o "$bc_f"
    $LLVM_DIR/bin/opt -S -p=mem2reg "$bc_f" -o "$bc_f"
  done
done
fi
