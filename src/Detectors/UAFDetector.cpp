...
void UAFDetector::initHandler(SVFModule *module) {
...
bool UAFDetector::runFSMOnModule(SVFModule *module) {
    /// start analysis
...
void UAFDetector::reportUAF(const ICFGNode *node, const Z3Expr &branchCond, const KeyNodesSet &keyNodesSet) {
    _stat->incBugNum();
...
void UAFDetector::validateSuccessTests(UAF_TYPE uafType, const ICFGNode *node) {
...
void UAFDetector::validateExpectedFailureTests(UAF_TYPE uafType, const ICFGNode *node) {
    const CallICFGNode *callBlockNode = dyn_cast<CallICFGNode>(_curEvalICFGNode);
    if (!callBlockNode) return;
    Set<const SVFFunction *> functionSet;
    _ptaCallgraph->getCallees(callBlockNode, functionSet);
